...
def checkRemaining(board,nums):
	totalRemaining = 0
	for row in board:
		for num in row:
			if num not in nums:
				totalRemaining += num
	print(totalRemaining*nums[-1])
...
for n in nums:
	...
	for board in boards:
		if winningCol(board,winningNums) or winningRow(board,winningNums):
			boards.remove(board)
			winningBoards.append({
				"board" : board,
				"winningSoFar" : winningNums 
				})
	if len(boards) == 0:
		break
			
checkRemaining(winningBoards[-1]["board"],winningBoards[-1]["winningSoFar"])
